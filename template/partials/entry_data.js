...
return unitLib.data.execSql(mysqlConnectionNo, `{{{replaceContext (replace rule.arguments.[0] "'" "\'") '`'}}}`)
{{/if}}
{{!-- SUB_TYPE_REDIS --}}
...
return unitLib.data.execRedis(redisConnectionNo, {{{replaceContext (JSONstringify rule.arguments.[0]) '"'}}})
{{/if}}
{{> data_send rule=this}}